...
namespace Landis.Util
{
	public static class Bool
	{
		...
		{
			StreamReader strmReader = new StreamReader(path);
			...
			if (trueChars == null)
				throw strmReader.MakeException("Expected a line starting " +
				                               "with \"true-chars =\"");
			...
			try {
				...
				while ((line = strmReader.ReadLine()) != null) {
					Match match = pattern.Match(line);
					if (! match.Success)
						throw strmReader.MakeException("Expected a line " +
						                               "starting with " +
						                               "\"row =\" or " +
						                               "\"row # =\"");
					...
					if (rows.Count == 1) {
						/*
						if (row.Length == 0)
							throw strmReader.MakeException("Expected at " +
							                               "least 1 " +
							                               "character after " +
							                               "the \"=\"");
						else */
						...
						string pluralSuffix = expectedRowLength == 1 ? ""
																	 : "s";
						throw strmReader.MakeException(string.Format(
						                               "Expected {0} " +
						                               "character{1} after " +
						                               " the \"=\"",
						                               expectedRowLength,
				...
				throw strmReader.MakeException(e.Message);
			}
		}
	}
}
