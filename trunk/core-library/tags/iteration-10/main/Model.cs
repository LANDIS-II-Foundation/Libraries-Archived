...
namespace Landis
{
	public static class Model
	{
		...
		private static float cellLength;
		...
		public static void Run(string scenarioPath)
		{
			...
			using (Landscape.IInputGrid<bool> grid = ecoregionsMap.OpenAsInputGrid()) {
				Log.Info("Map dimensions: {0}, {1:#,##0} cell{2}", grid.Dimensions,
				         grid.Count, (grid.Count == 1 ? "" : "s"));
				landscape = new Landscape.Landscape(grid);
			}
			Log.Info("Sites: {0:#,##0} active ({1:p1}), {2:#,##0} inactive ({3:p1})",
			         landscape.ActiveSiteCount, ((double)landscape.ActiveSiteCount)/landscape.Count,
			         landscape.InactiveSiteCount, ((double)landscape.InactiveSiteCount)/landscape.Count);
		...
		public static IEnumerable<MutableActiveSite> DisturbedSites()
		{
			// TODO
			return null;
		}
