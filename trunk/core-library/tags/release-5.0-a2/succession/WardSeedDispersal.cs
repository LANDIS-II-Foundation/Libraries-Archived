	...
	public static class WardSeedDispersal
	{
		                             ...
		                             ActiveSite site)
		{
			...
			foreach (NeighborInfo neighborInfo in neighborhoods[species.Index]) {
				Site neighbor = site.GetNeighbor(neighborInfo.RelativeLocation);
				if (neighbor == null) {
					offLandscapeCount++;
					continue;
				}
				ActiveSite activeNeighbor = neighbor as ActiveSite;
				if (activeNeighbor != null) {
					activeCount++;
					if (Reproduction.MaturePresent(species, activeNeighbor) &&
					    	Util.Random.GenerateUniform() < neighborInfo.DistanceProbability) {
						found = true;
						break;
					}
				}
